#code by Jacob Perkins, featured in Python Text Processing with NLTK 2.0 Cookbook
import itertools
...
def conll_tag_chunks(chunk_sents):
        ...
        [[('DT', 'B-NP'), ('NN', 'I-NP')]]
        '''
        tagged_sents = [nltk.chunk.util.tree2conlltags(tree) for tree in chunk_sents]
...
class TagChunker(nltk.chunk.ChunkParserI):
        ...
        def parse(self, tagged_sent):
                ...
                return nltk.chunk.util.conlltags2tree([(w,t,c) for (w,(t,c)) in wtc])

def chunk_trees2train_chunks(chunk_sents):
        tag_sents = [nltk.chunk.util.tree2conlltags(sent) for sent in chunk_sents]
...
class ClassifierChunker(nltk.chunk.ChunkParserI):
        ...
        def parse(self, tagged_sent):
                ...
                return nltk.chunk.util.conlltags2tree([(w,t,c) for ((w,t),c) in chunks])
...
class PersonChunker(nltk.chunk.ChunkParserI):
        ...
        def parse(self, tagged_sent):
                ...
                return nltk.chunk.util.conlltags2tree(iobs)
...
def ieer_chunked_sents(tag=nltk.tag.pos_tag):
        for doc in ieer.parsed_docs():
                ...
                yield nltk.chunk.util.conlltags2tree(tagged)
