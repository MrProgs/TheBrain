...
def first_chunk_index(chunk, pred, start=0, step=1):
        ...
        returns True.
        >>> first_chunk_index([('61', 'CD'), ('years', 'NNS')], lambda (word, tag): tag == 'CD')
        0
        >>> first_chunk_index([('61', 'CD'), ('years', 'NNS')], lambda (word, tag): tag == 'NNS')
        1
        >>> first_chunk_index([('61', 'CD'), ('years', 'NNS')], lambda (word, tag): tag == 'CD', start=1, step=-1)
        0
        >>> first_chunk_index([('61', 'CD'), ('years', 'NNS')], lambda (word, tag): tag == 'VB')
        '''
        l = len(chunk)
...
def convert_tree_nodes(tree, mapping):
        '''
        >>> convert_tree_nodes(Tree('S', [Tree('NP-SBJ', [('foo', 'NN')])]), {'NP-SBJ': 'NP'})
